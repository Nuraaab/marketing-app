...
class ServiceController extends Controller
{
    public function serviceSection(){
        ...
        return view('admin.dashboard.service.service_section', compact('staticSiteData', 'serviceData'));
    }

    public function viewServices(){
        $data['services'] = Service::all();
        return view('admin.dashboard.service.services', $data);
    }
    ...
    public function editService(Request $request, $id){
        ...
        try{
            ...
            $service->button_url = $button_url;
            if ($service->save()) {
                return redirect()->back()->with('success', 'Service data updated successfully!');
            } else {
                return redirect()->back()->with('error', 'An error occurred while saving the service.');
            }
    ...
    public function destroyService($id){
        $service = Service::findOrFail($id);
        $imagePath = public_path('admin/service_image/' . $service->image);
        if (file_exists($imagePath)) {
            unlink($imagePath);
        }
        $iconPath = public_path('admin/service_image/' . $service->icon);
